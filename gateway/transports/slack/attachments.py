...
def download_file(file: SlackInboundFile, token: str) -> bytes | None:
    ...
    Slack's own hosts — an off-Slack ``url_private`` is refused outright, and a
    redirect that leaves Slack is fetched without the ``Authorization`` header.
    The response is size-capped so a huge upload cannot exhaust memory (the file
    is dropped, not truncated, past the cap).
    """
