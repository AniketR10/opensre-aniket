"""Tests for app/utils/twilio_delivery.py — unified Twilio transport."""
...
def test_post_twilio_sms_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    success, error, sid = post_twilio_message(
        channel="sms",
        to="+14155550000",
        text="ping",
        account_sid="AC1",
...
def test_post_twilio_whatsapp_adds_prefix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert captured["data"]["To"] == "whatsapp:+14155550000"
    assert captured["data"]["From"] == "whatsapp:+14155238886"


def test_post_twilio_whatsapp_preserves_existing_prefix(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured = _patch_post(monkeypatch, _Resp({"sid": "SM4"}))
...
def test_post_twilio_status_callback_passes_through(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured = _patch_post(monkeypatch, _Resp({"sid": "SM5"}))
...
def test_post_twilio_missing_sender_fails() -> None:
    success, error, sid = post_twilio_message(
        channel="sms",
        to="+14155550000",
        text="hi",
...
def test_post_twilio_transport_failure_redacts_token(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_post_twilio_api_error_returns_message(monkeypatch: pytest.MonkeyPatch) -> None:
