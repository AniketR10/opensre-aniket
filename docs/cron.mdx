...
schedule. Use it for recurring prompt loops, GitHub PR standups, PostHog metric
reports, and work-item reminders — delivered automatically without manual CLI
runs.
...
| `--kind` | Yes | Task kind: `manual_loop`, `github_pr_sweep`, `posthog_metric_report`, `work_item_reminder`, `work_item_checkin` |
...
By default this delivers to every configured destination again, even ones
that already received the message. To recover a partial failure without
re-posting to channels that already got it, use `--failed-only` — it retries
only the destinations the most recent run failed at. With no prior run, or one
from before per-target history was tracked, `--failed-only` falls back to a
normal full run.
