...
| xAI Grok Build CLI| `grok-cli`| `grok login` or `XAI_API_KEY` | Grok Build CLI default                        | Grok Build CLI default                          |
...
# Optional overrides (all blank-by-default):
export GROK_CLI_MODEL=          # e.g. grok-build-0.1; unset → CLI's configured default
...
omits `-m` and the CLI uses its configured default. If `GROK_CLI_BIN` is unset, the binary
is resolved via `PATH` and known install locations.

Invocations run as `grok -p PROMPT --output-format plain --no-auto-update` (plus `-m MODEL`
when set), so each opensre call is a single non-interactive turn. The adapter deliberately
omits `--always-approve`: OpenSRE drives its own tools, so Grok is used purely as a text
responder and never auto-executes shell commands or file edits.

**Auth detection caveat:** Grok Build has no documented `grok auth status` command, so
detection is best-effort and prioritizes the subscription path. A persisted `grok login`
session credential file under `~/.grok/auth`, `~/.grok`, or `~/.grok-build` is treated as
authenticated; otherwise `XAI_API_KEY` counts as authenticated (headless / CI fallback).
`XAI_API_KEY` is a secret and is forwarded **only** to the Grok subprocess (never via the
shared CLI env allowlist), so it cannot leak into other CLI adapters. The runner re-verifies
auth at invoke time.
