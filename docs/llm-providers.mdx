...
detection is best-effort. `XAI_API_KEY` is treated as authenticated; otherwise a persisted
`grok login` session credential file under `~/.grok-build` or `~/.grok` counts as
authenticated. `XAI_API_KEY` is a secret and is forwarded **only** to the Grok subprocess
(never via the shared CLI env allowlist), so it cannot leak into other CLI adapters. The
runner re-verifies auth at invoke time.
