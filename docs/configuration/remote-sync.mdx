...
The environment variable replaces the stored list for that run rather than
adding to it.
...
Check a pattern took effect before trusting it — `status` counts what each
pattern currently holds back, which is the quickest way to catch a typo:
