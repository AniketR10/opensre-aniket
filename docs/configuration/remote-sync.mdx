...
that run rather than adding to it. Setting it to an empty value counts as not
setting it at all — the stored list stays in force — so a script that expands an
unset name cannot quietly turn your exclusions off. To sync a path you normally
exclude, remove it from the list rather than blanking the variable.
