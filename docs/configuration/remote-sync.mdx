...
```bash
OPENSRE_REMOTE_SYNC_EXCLUDE=none opensre remote-sync sync
```

Blanking the variable does **not** do this — an empty value counts as not
setting it at all, and your stored list stays in force. That is deliberate, so a
script expanding a name that turns out to be unset cannot quietly switch your
exclusions off and upload what you meant to keep local.
