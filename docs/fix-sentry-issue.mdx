...
| `CODING_AGENT` | Which coding backend implements the fix. Defaults to `pi` (the only one supported today). |
| `CODING_MODEL` | Model used for the fix (shared with the coding tool). Falls back to `PI_CODING_MODEL`. |
| `CODING_WORKSPACE` | Repository the agent edits. Defaults to the current directory. Falls back to `PI_CODING_WORKSPACE`. |
| `CODING_TIMEOUT_SECONDS` | Per-run timeout (default 600, clamped 60–1800). Falls back to `PI_CODING_TIMEOUT_SECONDS`. |
